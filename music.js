...
function getChannel(){
    $.ajax({
        ...
        success: function(response){
            ...
            $('.music-type').text(channelname);
            $('.music-type').attr('title',channelname);
            $('.music-type').attr('data-id',channelId);
            getmusic();
        }
    })
}
...
function getmusic(){
    $.ajax({
        ...
        Method: 'get',
        data:{
            'channel': $('.music-type').attr('data-id')
        },
        success: function (ret) {
            ...
            $('.music-author').text(author);
            $('.music-author').attr('title',author)
            $(".picture").css({
            ...
            if (!!lyr.lyric) {
                ...
                if(line != ""){
                    for(var i in line){//遍历歌词数组
                        ...
                        var value = line[i].replace(timeReg,"");// 纯歌词，把歌词前面的时间删除
...
function showLyric(){
    var liH = $(".lyric li").eq(5).outerHeight()-3; //每行高度  所显示的那一行的高度
    for(var i=0;i< lyricArr.length;i++){//遍历歌词下所有的li
        ...
        if ((curTime > curT) && (curT < nexT)){//当前时间 在 下一句时间 和 歌曲当前时间 之间的时候 就渲染 并滚动
...
$(".basebar").mousedown(function(ev){  //拖拽进度条控制进度
    var posX = ev.clientX;//鼠标指针相对于当前窗口的X
    var targetLeft = $(this).offset().left;
    var percentage = (posX - targetLeft)/400*100;//??? 这里的400指的是进度条的长度，已经播放的百分比
    myAudio.currentTime = myAudio.duration * percentage/100;//??.duration是指整个音频的长度  这里的100是个什么鬼???????????
});
function present(){
    var length = myAudio.currentTime/myAudio.duration*100;//100是什么鬼？？？？？？？？？？
...
$('.m-lyric').on('click',function(){
    ...
    if ($(this).hasClass('lyriced')) {
        $('.picture .music-lyric').css({'display':'block'})
    }else{
        $('.picture .music-lyric').css({'display':'none'})
    }
})
